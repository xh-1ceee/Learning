...
#define STACK_FENCEPOST 0xdeadbeef	// this is put at the top of the
					...
					// stack overflows
int threadIdMap[128];
Thread* allThreads[128];
int threadIDComp(void *target, void *data)
{
    int threadId = ((Thread *)target)->getUserProgramId();
...
Thread::~Thread()
{
    ...
    threadIdMap[UserProgramId] = 0;
    allThreads[UserProgramId] = NULL;
#ifdef USER_PROGRAM
    delete space;
#endif
...
{
    IntStatus oldLevel = interrupt->SetLevel(IntOff);
    currentThread->waitingProcessSpaceId = tid;
    //在终止队列中搜索tid
    Thread* thread;
    ...
    for(int i = 1;i<=listLength;i++)
    {
        ...
        if(thread==NULL) inTermi = false;
        if(thread->UserProgramId == tid)
        {
            inTermi = true;
            break;
        }
    ...
    //如果没有搜索到tid,则其还没有finish
    if(!inTermi)
    {
        waitingProcessSpaceId = tid;
        waitingList->Append((void*)this);
        currentThread->Sleep();
    }
    //否则的话，搜索到了tid
    currentThread->waitProcessExitCode = waitingThreadExitCode;
    scheduler->deleteTerminatedThread(tid);
